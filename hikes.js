...
function search() {
  ...
  let filteredHikes = hikes.filter((hike) => {
    return (
    hike.name.toLowerCase().includes(highQuery.toLowerCase()) ||
    hike.description.toLowerCase().includes(highQuery.toLowerCase()) ||
    hike.tags.some((tag) => tag.toLowerCase().includes(highQuery.toLowerCase()))
  );
  })
